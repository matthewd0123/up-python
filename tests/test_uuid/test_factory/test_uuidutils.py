...
class TestUUIDUtils(unittest.TestCase):
    ...
    #     self.assertEqual(DELAY_MS, UUIDUtils.get_elapsed_time(id), DELTA)
    ...
    def test_get_remaining_time(self):
        id: UUID = create_id()
        self.assertEqual(TTL, UUIDUtils.get_remaining_time(id, TTL), DELTA)
    ...
    def test_get_remaining_time_attributes(self):
        attributes: UAttributes = UAttributesBuilder.publish(build_source(), UPriority.UPRIORITY_CS0).withTtl(TTL).build()
        self.assertEqual(TTL, UUIDUtils.get_remaining_time(attributes), DELTA)
        time.sleep(DELAY_MS/1000)
        self.assertEqual(TTL - DELAY_MS, UUIDUtils.get_remaining_time(attributes), DELTA)
    ...
    def test_get_remaining_time_attributes_expired(self):
        attributes: UAttributes = UAttributesBuilder.publish(build_source(), UPriority.UPRIORITY_CS0).withTtl(DELAY_MS - DELTA).build()
        time.sleep(DELAY_MS/1000)
        self.assertFalse(UUIDUtils.get_remaining_time(attributes) is not None)
